...
class DNBSRUThread(QThread):
    ...
    def run(self):
        ...
        dnb_sru(self.query, self.metadata, self.base_url, self.progress_signal, self.filename, self.is_running)
...
class SRUQueryApp(QMainWindow):
    def __init__(self):
        ...
        layout.addWidget(QLabel("""Bitte wählen Sie im folgenden einen Katalog und ein Metadatenformat aus, 
        um die SRU-Schnittstelle der DNB abzufragen:"""))
        ...
        try:
            ...
            if cat_url and metadata and query:
                ...
                if result < 100000:
                    ...
                    self.warning_label.setText("""Warnung! Ihre Anfrage ergibt mehr als 100.000 Treffer! 
                                        Bitte teilen Sie Ihre Anfrage so auf, dass Sie immer nur 
                                        Anfragen mit maximal 100.000 Treffern stellen 
                                        (bspw. indem Sie Zeitabschnitte hinzufügen).""")
                    self.warning_label.setVisible(True)
                    self.download_button.setEnabled(False)
            else:
                self.results_label.setText(
                    "Bitte wählen Sie einen Katalog und ein Metadatenformat aus und geben Sie eine Suchanfrage ein.")
                self.download_button.setEnabled(False)
        except Exception as e:
